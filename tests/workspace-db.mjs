...
export async function createDatabase(operations=process.env.C360_TEST_OPERATIONS==='1'||process.env.C360_TEST_V14==='1'||process.env.C360_TEST_V15==='1'){
 const db=new PGlite();
 ...
 if(process.env.C360_TEST_V14==='1'||process.env.C360_TEST_V15==='1'){
 ...
 if(process.env.C360_TEST_V15==='1')await db.exec(await fs.readFile(path.join(appRoot,'supabase/migrations/20260907195118_vehicle_inspections_v15.sql'),'utf8'));
...
export async function rpcAs(db,id,name,args=[]){const names={workspace_snapshot:[],workspace_save:['text','jsonb','uuid'],workspace_import:['jsonb','uuid'],operations_snapshot:[],operations_save:['text','jsonb','uuid'],vehicles_snapshot:[],vehicle_history:['uuid','integer'],vehicle_save:['text','jsonb','uuid']};if(!(name in names))throw new Error('Unknown test RPC');return (await asUser(db,id,`select public.${name}(${names[name].map((t,i)=>'$'+(i+1)+'::'+t).join(',')}) as result`,args)).rows[0].result}
