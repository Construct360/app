...
export async function createDatabase(operations=process.env.C360_TEST_OPERATIONS==='1'||process.env.C360_TEST_V14==='1'||(process.env.C360_TEST_V15==='1'||process.env.C360_TEST_V16==='1')){
 ...
 if(process.env.C360_TEST_V19==='1')await db.exec(await fs.readFile(path.join(appRoot,'supabase/migrations/20260919183227_profiles_planner_handover_v19.sql'),'utf8'));
 if(process.env.C360_TEST_V20==='1')await db.exec(await fs.readFile(path.join(appRoot,'supabase/migrations/20260921181503_dashboard_assignments_notifications_v20.sql'),'utf8'));
...
export async function rpcAs(db,id,name,args=[]){const names={notifications_snapshot:['bigint','integer'],notifications_mark_read:['bigint','boolean'],annual_leave_save:['text','jsonb','uuid'],scaffold_review_alerts:[],job_resources_snapshot:['uuid'],job_file_save:['text','jsonb','uuid'],scaffold_snapshot:[],scaffold_history:['uuid'],scaffold_save:['text','jsonb','uuid'],timesheets_snapshot:['date'],timesheet_history:['uuid'],timesheet_save:['text','jsonb','uuid'],workspace_snapshot:[],workspace_save:['text','jsonb','uuid'],workspace_import:['jsonb','uuid'],operations_snapshot:[],operations_save:['text','jsonb','uuid'],vehicles_snapshot:[],vehicle_history:['uuid','integer'],vehicle_save:['text','jsonb','uuid']};if(!(name in names))throw new Error('Unknown test RPC');return (await asUser(db,id,`select public.${name}(${names[name].map((t,i)=>'$'+(i+1)+'::'+t).join(',')}) as result`,args)).rows[0].result}
