...
function openTimesheet(id=null,mode='edit'){
 if(mode!=='view')prepareDetailEditor('timesheetDialog');
...
function closeTimesheet(){if(timesheetBusy)return;if(timesheetDirty&&!window.confirm('Discard unsaved changes? Choose Cancel to keep editing and Save draft.'))return;timesheetHistoryGeneration++;timesheetDirty=false;$('timesheetDialog').close();if(readPanel?.dialog.id==='timesheetDialog'){closeReadPage();render()}}
